...
describe("mergeSort", () => {
    ...
    it("sorts numbers descending when ascending=false", () => {
        const input = [5, 1, 4, 2, 8];
        const out = mergeSort(input, { ascending: false });
        expect(out).toEqual([8, 5, 4, 2, 1]);
    });

    it("handles already sorted arrays", () => {
        const input = [1, 2, 3, 4];
        expect(mergeSort(input)).toEqual([1, 2, 3, 4]);
    });
    ...
    it("handles duplicates", () => {
        ...
        expect(mergeSort(input)).toEqual([1, 2, 2, 3, 3]);
    });

    it("handles negative numbers", () => {
        const input = [0, -10, 5, -3];
        expect(mergeSort(input)).toEqual([-10, -3, 0, 5]);
    });
    ...
    it("uses custom comparator when provided", () => {
        ...
        const out = mergeSort(input, { compare: (a, b) => a.v - b.v });
    ...
    it("throws when array is not an Array", () => {
        expect(() => mergeSort("nope")).toThrow();
        expect(() => mergeSort({})).toThrow();
        expect(() => mergeSort(null)).toThrow();
    });

    it("throws when ascending is not boolean", () => {
        expect(() => mergeSort([3, 2, 1], { ascending: "false" })).toThrow();
        expect(() => mergeSort([3, 2, 1], { ascending: 1 })).toThrow();
    });

    it("throws when compare is not a function", () => {
        expect(() => mergeSort([3, 2, 1], { compare: 123 })).toThrow();
        expect(() => mergeSort([3, 2, 1], { compare: "cmp" })).toThrow();
    });

    it("throws when compare does not return a valid number (ComparatorError)", () => {
        const badCompare = () => "nope";
        expect(() => mergeSort([3, 2, 1], { compare: badCompare })).toThrow();
    });
});
